...
**Update (2026-07-14) — item 2 confirmed on GNOME/Wayland (Ubuntu):** ran `npm run dev` (unmodified code, no icon changes) on a GNOME/Wayland desktop with the `ubuntu-appindicators@ubuntu.com` extension enabled. `busctl --user get-property org.kde.StatusNotifierWatcher /StatusNotifierWatcher org.kde.StatusNotifierWatcher RegisteredStatusNotifierItems` confirmed the running Electron process registered a `StatusNotifierItem`, and the user visually confirmed the tray icon appeared. No code changes were needed — the app worked as-is.

Note for future debugging on this hardware: an earlier attempt to test this same setup failed silently (Electron processes exited immediately with no output, not even the GNOME `console.warn`). Root cause was `ELECTRON_RUN_AS_NODE=1` set in that shell session, which makes `require('electron')` return a path string instead of the API (`electron.app` is `undefined`), crashing before `app.whenReady()`. This is unrelated to Prismly's code — it silently broke every prior test in that shell. Anyone debugging Linux tray behavior via an agent/CI shell should `unset ELECTRON_RUN_AS_NODE` first and confirm with `echo $ELECTRON_RUN_AS_NODE` before concluding anything about the app itself. Items 1, 3, and 4 above remain unverified.
